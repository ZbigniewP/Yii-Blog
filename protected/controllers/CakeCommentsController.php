...
class CakeCommentsController extends Controller
{
	...
	public function actionUpdate()
	{
		...
		if (isset($_POST['CakeComments'])) {
			$model->attributes = $_POST['CakeComments'];
			if ($model->save())
	...
	{
		$dataProvider = new CActiveDataProvider('CakeComments', array(
			'criteria' => array(
				'with' => 'post',
				'order' => 't.status, t.created DESC',
			),
		));

		$this->render('//cakecomments/index', array('dataProvider' => $dataProvider));
	}
	...
	public function loadModel()
	{
		if ($this->_model === null) {
			if (isset($_GET['id']))
				$this->_model = CakeComments::model()->findbyPk($_GET['id']);
			if ($this->_model === null)
